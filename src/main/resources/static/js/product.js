...
function addACategory() {
	let section = document.getElementById('addSection').value; // 섹션 값을 가져옵니다.
	let category = document.getElementById('categoryName').value; // 선택된 카테고리 값을 가져옵니다.

	// '새 카테고리 추가' 선택 시
	if (category === 'etc') {
		// 새 카테고리 입력란을 활성화하고 배경색을 투명으로 설정
		$('#addNewCategory').removeAttr('disabled').css('background-color', 'transparent').focus();

		// 입력란에서 포커스가 벗어났을 때의 이벤트 처리
		$('#addNewCategory').off('blur').on('blur', function(){
			category = $('#addNewCategory').val(); // 입력된 새 카테고리 값을 가져옵니다.
					...
					if (data.errorMessage) {
						Swal.fire({
							...
							if (result.isConfirmed) {
								$('#addNewCategory').val('').attr('disabled', true).css('background-color', '#E5E5E5');
								$('#categoryName').val(''); // 셀렉트 박스를 초기 상태로 설정
							}
						});
					} else {
						// 성공 시 추가 로직 구현
