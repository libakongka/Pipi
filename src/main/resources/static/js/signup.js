/* 아이디 유효성 검사 및 중복 확인 */
$('#searchMemberId').on('click', function() {
	const regexp = /^[a-z0-9]{4,12}$/;
	const memberId = $('[name=username]').val();
	...
	if(memberId != '') {
		$.ajax({
			...
			success : function(result){
				if(result) { // 서버로부터 true를 받으면, 중복된 아이디
					$('#checkIdMsg').text('이미 사용 중인 아이디입니다').css('color', 'red');
					$('#registBtn').attr('disabled', true);
				} else { // 서버로부터 false를 받으면, 사용 가능한 아이디
					if(!regexp.test(memberId) || memberId.length < 4) {
						$('#checkIdMsg').text('4~12자의 영문/숫자 조합하여 입력하세요').css('color', 'red');
						$('#registBtn').attr('disabled', true);
					} else {
						// $('#checkIdMsg').text('사용 가능한 아이디입니다.');
		...
		$('#checkIdMsg').text('아이디는 필수 입력 항목입니다').css('color', 'red');
		$('#registBtn').attr('disabled', true);
	}
});
...
$('#email').on('propertychange change keyup paste input focusout', function(){
	console.log($(this).val()); // 현재 입력된 이메일 값 출력
